...
export interface Violation {
  statute: string;           // e.g. "15 USC §1692g(b)"
  act: "FDCPA" | "FCRA" | "ECOA" | "GLBA" | "CFPA" | "UDAAP";
  section: string;            // human-readable subsection name
  description: string;       // what the law prohibits/requires
  elements: string[];         // what the consumer must prove
  penalties: string;         // statutory damages / remedies
  deadlines?: string;        // relevant time limits
  violations: string[];      // common ways collectors/reporters break this
  demandLanguage: string;    // suggested demand/cease language
}

export const FDCPA_VIOLATIONS: Violation[] = [
  {
...
export const FCRA_VIOLATIONS: Violation[] = [
  {
...
export const ALL_VIOLATIONS: Violation[] = [...FDCPA_VIOLATIONS, ...FCRA_VIOLATIONS];

export interface AnalysisResult {
  matches: Array<{ violation: Violation; confidence: number; reasoning: string }>;
