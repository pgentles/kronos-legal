...
import { ALL_VIOLATIONS, Violation } from './data/statutes.js';
...
const queryHistory: Array<{ type: string; timestamp: string; path: string }> = [];
const MAX_QUERIES = 500;

function recordQuery(type: string, path: string) {
  queryHistory.push({ type, timestamp: new Date().toISOString(), path });
  if (queryHistory.length > MAX_QUERIES) queryHistory.shift();
}
