...
export function analyzeText(text) {
    ...
    for (const p of PATTERNS) {
        if (p.pattern.test(text) || p.pattern.test(lower)) {
            for (const statuteId of p.violations) {
                if (matchedStatutes.has(statuteId))
                    continue;
    ...
    for (const v of ALL_VIOLATIONS) {
        if (matchedStatutes.has(v.statute))
            continue;
...
function generateAdvice(matches) {
    if (matches.length === 0)
        return 'Provide more details about the specific illegal or inaccurate conduct experienced.';
    const top = matches[0];
